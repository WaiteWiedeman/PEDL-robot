function MakeVideo(sysParams, t, x, xp, ref,endeff,endeffp, tSpan)
     % Set up video
    ...
    for n = idx1:idx2
        ...
        set(gca,'Position',[0.07,0.55,0.4,0.15]);
        xlabel("Time (s)",'FontSize', 15);
        grid on;
        ...
        set(gca, 'FontName', "Arial")
        axis padded
        set(gca,'Position',[0.55,0.55,0.4,0.15]);
        grid on;
        ...
        annotation('textbox', [0.1, 0.1, 0.3, 0.2], ...
        ...
        daspect([1 1 1])
        set(gca,'Position',[0.05,0.1,0.8,0.4]);

        tObj = title("System at "+num2str(tSpan(2)-tSpan(1))+" second", "FontName", "Arial","FontSize",15);
        tObj.Position(1) = -3.0;
        legend([h1 h2 h3], "FontName","Arial", "FontSize", 15, 'Position', [0.1, 0.3, 0.2, 0.1]);
