function xdot = robot_xdot(x, F, sysParams)
    ...
    % compute qdd: D*qdd = F - C*thd - G, using linsolve
    D = [                M0+M1+M2                     -((M1*l1+M2*L1)*sin(th1)+M2*l2*sin(th1+th2))         -M2*l2*sin(th1+th2)
        -l2*M2*sin(th1+th2)-(L1*M2+l1*M1)*sin(th1)     L1^2*M2+l1^2*M1+l2^2*M2+2*L1*l2*M2*cos(th2)       l2^2*M2+L1*l2*M2*cos(th2)
                   -l2*M2*sin(th1+th2)                           l2^2*M2+L1*l2*M2*cos(th2)                         M2*l2^2        ];
